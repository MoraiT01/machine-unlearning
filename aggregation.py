...
def main(
    ...
    for filename in filenames:
        outputs.append(
            np.load(
                os.path.join("containers/{}/outputs".format(container), filename),
                allow_pickle=True,
            )
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("Aggregations of all the predictions of all shards")
    parser.add_argument(
        "--strategy", 
        default="uniform", 
        help="Voting strategy, default uniform"
    )
    parser.add_argument(
        "--container", 
    ...
    parser.add_argument(
        "--baseline", 
        type=int, 
        help="Use only the specified shard (lone shard baseline)"
    )
    parser.add_argument(
        "--label", 
        default="latest", 
        help="Label, default latest")
