    ...
    if args_shard != None:
        ...
        # Else run PLS-GAP algorithm to find a low cost split.
            ...
            def mass(index):
                ...
                if distribution.split(":")[0] == "pareto":
                    a = (
                        float(distribution.split(":")[1])
                        if len(distribution.split(":")) > 1
                        else 1.16
    ...
    if args_requests != None:
        if distribution == "reset":
            ...
            if distribution.split(":")[0] == "exponential":
                ...
                all_requests = np.random.exponential(1 / lbd, (args_requests,))
            if distribution.split(":")[0] == "pareto":
                a = (
                    float(distribution.split(":")[1])
                    if len(distribution.split(":")) > 1
                ...
                all_requests = np.random.pareto(a, (args_requests,))
            else:
                all_requests = np.random.randint(0, datasetfile["nb_train"], args_requests)
