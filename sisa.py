...
def main(
    ...
    model = model_lib.Model(input_shape, nb_classes, dropout_rate=dropout_rate)
    model.to(device)
    ...
    if train:
        shard_size = sizeOfShard(container, shard)
        slice_size = shard_size // slices
        avg_epochs_per_slice = (
            2 * slices / (slices + 1) * epochs / slices
        )
        loaded = False
        ...
        for sl in range(slices):
            ...
            slice_hash = getShardHash(
                container, label, shard, until=(sl + 1) * slice_size
            )

            # If checkpoints exists, skip the slice.
            if not os.path.exists(
                "containers/{}/cache/{}.pt".format(container, slice_hash)
            ):
                # Initialize state.
                elapsed_time = 0
                start_epoch = 0
                ...
                if not loaded:
                    ...
                    recovery_list = glob(
                        "containers/{}/cache/{}_*.pt".format(container, slice_hash)
                    )
                    if len(recovery_list) > 0:
                        ...
                        with open(
                            "containers/{}/times/{}_{}.time".format(
                                container, slice_hash, start_epoch
                            ),
                            "r",
                        ) as f:
                    ...
                    # If there is no recovery checkpoint and this slice is not the first, load previous slice.
                        ...
                        previous_slice_hash = getShardHash(
                            container, label, shard, until=sl * slice_size
                        )
                ...
                for epoch in range(start_epoch, slice_epochs):
                    ...
                    for images, labels in fetchShardBatch(
                        ...
                        batch_size,
                        dataset,
                        until=(sl + 1) * slice_size if sl < slices - 1 else None,
                    ):
                    ...
                    if (
                        chkpt_interval != -1
                        and epoch % chkpt_interval == chkpt_interval - 1
                    ):
                        # Save weights
                        torch.save(
                            model.state_dict(),
                            "containers/{}/cache/{}_{}.pt".format(
                                container, slice_hash, epoch
                            ),
                        )
                        ...
                        if os.path.exists(
                            "containers/{}/cache/{}_{}.pt".format(
                                container, slice_hash, epoch - chkpt_interval
                            )
                        ):
                            os.remove(
                                "containers/{}/cache/{}_{}.pt".format(
                ...
                with open(
                    "containers/{}/times/{}.time".format(container, slice_hash), "w"
                ) as f:
                    f.write("{}\n".format(train_time + elapsed_time))
                ...
                if os.path.exists(
                    "containers/{}/cache/{}_{}.pt".format(
                        container, slice_hash, epochs - chkpt_interval
                    )
                ):
                    os.remove(
                        "containers/{}/cache/{}_{}.pt".format(
    ...
    if test:
        ...
        for images, _ in fetchTestBatch(dataset, batch_size):
            # Convert data to torch format and send to selected device.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", 
        default="purchase", 
        help="Architecture to use, default purchase"
    )

    parser.add_argument(
        "--train", 
        action="store_true", 
        help="Perform SISA training on the shard"
    )
    parser.add_argument(
        "--test",
        action="store_true", 
        help="Compute shard predictions")
    ...
    parser.add_argument(
        "--shard", 
        type=int, 
        help="Index of the shard to train/test")
    parser.add_argument(
        "--slices",
        default=1, 
        type=int,
        help="Number of slices to use, default 1"
    )
    parser.add_argument(
        "--dataset",
